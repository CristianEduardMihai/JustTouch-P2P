...
**JustTouch** is a cross-platform file sharing app for Android, iOS(Work in progress), Windows, macOS, and Linux. Share files with just a tap on mobile devices via NFC, or use QR codes on any platform. Pick one or more files, generate a web link, and send it to another device. The other person gets the link and downloads the file directly. No pairing, no logins, no cloud, no app installation required.
...
- **QR code/Link sharing**: Scan codes or copy links to share files with desktops.
